...
It is a plugin. Community is constsntly dweveloping packages which makes some R code easier, extend R's capabilites. R studio is the phone and packages are the apps. 
...
library(tidyverse)
#umbrella package, it loads a number of packages 
```

Question: What happens in the console when you load a package?

> Answer: it downloads the packages from the library so you can use them, and will give you error signals that you should pay attention to if certain packages have similar commands and may mask one another 
...
#Set a single directory to be the owrking directory and everything will be relative to that. Uusually wherever your R file is. Can find where this is b typing >getwd() 
...
#serwd() can set your path to wherever you want and instead of making it a relative path it becomes an absoutle path so be careful 
...
here()
#this is usually more accurate than getwd() 
```
...
#we always use csv for downloads 
...
#spanned over three lines, when you knit th files its nice to not have these extend outside the page
#command is not going to work if the folder is renamed because it is an absolute path
...
USGS.flow.data <- read.csv(
  ...
  stringsAsFactors = TRUE)   
#see the here command. putting contents into a usgs.flow data object. 
#strings as factor all the charcahters become a factor but treated by r as caegorical data. 
#want to add these stringsasfactors=TRUE whenever we read a dataset. 
...
> ANSWER: find it in metadata in data in files. Eno river at Hillsborough, NC. Collected 1928. Includes info on coloumns. 
...
```{r Exploring data}
#View(USGS.flow.data) #want to comment out Views before knitting 
# Alternate option: click on data frame in Environment tab

class(USGS.flow.data) #what kind of variable is this variable? 
colnames(USGS.flow.data) #tells you the coloumn names 
...
#this above is a vector with a bunch of strings and the length of this above vector has to match the vector in the console 
str(USGS.flow.data) #structure, can also use glimpse(USGS.flow.data)
dim(USGS.flow.data) #the dimensions, rows and coloumns of our data frame 
length(USGS.flow.data) #tells us how many coloumns in our data frame 

head(USGS.flow.data) #spits out the first 5 records of our console 
head(USGS.flow.data, 10) #this would show 10 instead 
tail(USGS.flow.data, 5) #shows min mean and max of first 5 records 
USGS.flow.data[30000:30005, c(3, 8, 14)] #look at rows 30000 and 30005 in coloumns 3, 8 and 14 

class(USGS.flow.data$datetime) #this shows up as a facotr but actually want it to be an object 
class(USGS.flow.data$discharge.mean) #class of the mean are numeric 
class(USGS.flow.data$gage.height.mean) #class of specific coloumns 

summary(USGS.flow.data)  #could point to column only with $
#choose all the numeric coloumns and will give us summary stats on them 
...
```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #na.omit gets rid of missing data 
dim(USGS.flow.data)
dim(USGS.flow.data.complete)

mean(USGS.flow.data.complete$discharge.mean) #original data frame. cant actually calculate the mean when it has N/A values but once you plug in the data set from omit it is fine. got to get rid of missing data first  
sd(USGS.flow.data.complete$discharge.mean)
summary(USGS.flow.data.complete$discharge.mean) #this is the summary with the omitted data.

#missing data is a thing and a thing you have to pay attention to because it happens often 
...
```{r}
#for example, for site number in this USGS data frame the site number is numeric but we want it to be charcahters so we need to coerce it into a different factor 
...
USGS.flow.data2  <- read.csv(
  file = here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv"),
  stringsAsFactors = TRUE) 
colClasses = c('site_number =factor') #this will override the default how r will import a column in a data frame
```
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") 
  #this is based off the metadata of USGS which formats the date this way, pay attention! 
...
#want to get rid of the 20 in the 2027 since this was taken in 1927 and replace it with 19. 191226 (our last observation in 1900's will be december 26 1900)
...
write.csv(
  ...
  row.names=FALSE)
#goes in processed data folder for saving. use 'here' function to do this 
```
