...
#string as True helps read things as charcaters 

colnames(NTL.phys.data) #giving names of all the columns reported, multiple dates because each depth of the lake is measured 
head(NTL.phys.data) #summary of each of the columns 
summary(NTL.phys.data) 
str(NTL.phys.data) #data types for each column 
dim(NTL.phys.data)

class(NTL.phys.data$sampledate) #shows that date class needs to be reformatted bc its a factor 
# Format sampledate as date
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y")
#changed the data from factor to date 
```
...
```{r, results = "hide"}
vignette("dplyr")
#three categores: rows: filter() chooses rows based on coloumn values, slice (), arrange (), etc, run above code for more info 
```
...
#filtering only the rows that have the element 0 on column depth 

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) #same thing as above but easier perhaps 
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25)
#filter function, first argument data set next is the arugment. 
...
#this second one is exclusing the other lakes one by one so is not easier. 

NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
#just want to extract paul and peter lake 
# Choose a range of conditions of a numeric or integer variable, this is a vector where you can just type %in% and the filter names 
...
#numeric variable, all does the same thing ^ 
...
```{r arranging data}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) #always arranges in ascending order unless otherwise stated 
...
#the colon symbol in between sample data and temperature will select all the coloumns in between these two 
```
### Mutate
...
# find out the start and end dates of the dataset or find an interval. First row is 1 and last row is 21613 
...
#can also do it this way ^. have to provide entire code with coloumn sample date 
```
...
#cleaner way to do everything we did before 
...
#save in csv for further use. must proivde file name path 
```
