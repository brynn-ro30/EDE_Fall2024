...
#use this link for a decision tree of what you should use for data visualization 
...
```{r, fig.height = 3, fig.width = 5, warning = FALSE, fig.cap="Scatter Plot"}
# Scatterplot
...
print(O3plot) #add two plots if you want and/or other layers you want and then just printing that object. The first two lines of code are saving all the layers together, and the print actually generates the plot 
...
# Filter dataset within plot building and facet by multiple variables. How to select only certain sites if you dont want all of them. 
PMplot.faceted2 <- 
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #if you do not specify then your data points will be plotted from 0 up, but we want it to show depth so we reverse it 
print(tempvsdepth)
...
print(Nutrientplot4) #dodge makes sure there are no overlying points, binaxis how the dots are stacking over which axis, stackdir: the direction of the stack 
...
print(Nutrientplot6)
#no legend, dont know what they are, x axis isnt just tn_ug so its not accurate. Use gathered data set instead 
