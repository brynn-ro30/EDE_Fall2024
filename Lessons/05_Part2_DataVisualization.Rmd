...
O3plot <- ggplot(EPAair) + #grey background white gridlines 
...
O3plot2 <- ggplot(EPAair) + #default gg plot theme (white backgorund, grey gridlines)
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw()
print(O3plot2)

O3plot3 <- ggplot(EPAair) + #classic dark on light 
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light()
print(O3plot3)

O3plot4 <- ggplot(EPAair) + #class like theme with x and y axis lines and no gridlines 
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic()
print(O3plot4)
```
...
theme_set(mytheme) #set theme for all plots in the future 
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #lty species the type of line. lty 2 is a dashed line
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + #add points, alpha is transparency and then the size 
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +   # hjust = 1 means left align #x and y describes where text will be printed 
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), #first and last date you want to be plotted 
