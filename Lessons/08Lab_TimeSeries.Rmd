...
#visualization is not always enough for data, sometimes you need stationary data 
...
BR_Rain.ts <- ts (BR_complete$Rainfall, start=c(f_year, f_month), frequency=12)
...
BR_Rainfall.ts_Decomposed <- stl(BR_Rain.ts, s.window =12)
...
f_timeseries <- timeseries(first(BR_Temp.ts_Decomposed$time.series))
f_year <-year(first(BR_complete$Date))
