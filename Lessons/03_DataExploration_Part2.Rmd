...
>Answer: to better know the next steps to analyze it and understand it 
...
USGS.flow.data <- read.csv(
  ...
  #want to make site number a factor 
...
ggplot(data = USGS.flow.data, aes(x = discharge.mean.approval)) +
  geom_bar()
#gives r a graph paper, aes is the aeshtetics of how to do that, what we want it to look like. 
```
...
```{r, fig.height = 3, fig.width = 4}
# x axis is discharge mean 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

# is the width of the histogram bar 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 1000)

# can change the number of bins, or removes uneccesary bins with no data 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# we are creating a new canvas
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500)) 
#full spread of data, focuses on values up to 500 
  
# looking at different value here 
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#this ^ will help your knitting document know how to format it 
#creating new plot based on usgs flow data, have three geomes now. like putting paper down and then drawing stuff on it.lty= line type in this case dashed 
ggplot(USGS.flow.data) +
...
#color is inside the aes which means they will apply some sort of attribute to the plot based off data frame, and in this case its going to be categorical. 
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#gives us boxplots, 3, because it is assigning one box plot to all the things without value. the big fat line in the box plot is the median. the box itself is showing the IQR. 
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#when x value is continous. has two continous values here. can see the relationship across bins in mean discharge height 
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#violin plot yeeeeeeeeeeeeeeeeeeeeet
ggplot(USGS.flow.data) +
...
Scatter plot is good for looking at correlation between variables 
```{r, fig.height = 3, fig.width = 4}
ggplot(USGS.flow.data) +
  geom_point(aes
             (x = discharge.mean, 
               y = gage.height.mean))
#date time shows gaps in data set. put date time in x its good for time series analysis 
ggplot(USGS.flow.data) +
