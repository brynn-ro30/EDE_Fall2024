...
#What is its CRS again? do $ colomn name to look @ a specific variable 
...
#Create a map
mapView(un_sf) + mapview(un_sf, col.region = "yellow")

ggplot() + 
  geom_sf(date=huc8_sf, fill="grey") 
...
#Tidyverse version of the join, tidier version of the above
...
#Update the area the features, now that some are clipped. the area remained the 
#same after clipping them so we need to adjust the area/acres 
...
triCo_intersect <- counties_sf_utm %>%  
  st_filter(triCo, .predicate = st_intersects)

myMap2 + mapview(triCo_intersect, alpha.regions = 0, color='red')

triCo_counties_sf <- triCo %>% st_intersection(counties_sf_utm)
triCo_counties_sf <- triCo_counties_sf %>% 
  mutate(Area_m2 = as.numeric(st_area(neuse_counties_sf$geometry)))
mapview(triCo_counties_sf, zcol='Area_m2')
...
leaflet() %>% addTiles() %>% #canvas 
  addPolygons(data=counties_WGS84,weight=1,color='red') %>% #polygons + polygon source 
