...
summary(NTL.phys.data.PeterPaul$comments) #need to convert to a date again :/, always need to process date object 
...
NTL.phys.data.PeterPaul.subset <- select(NTL.phys.data.PeterPaul, 
                                         lakename:irradianceDeck) #range specification, will choose every column between these 
...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration") #take all nutrient columns and gathering it into these two nutrient and concentrations columns 
...
NTL.PeterPaul.summaries <- 
  NTL.phys.nutrient.data.PeterPaul %>% #filter by depth
  filter(depth == 0) %>%
  group_by(lakename, month) %>% #group lake name n month
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #filter NA's
