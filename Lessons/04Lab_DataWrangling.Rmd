...
NTL.phys.data <- read.csv(
  file=here::here("Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv"), 
...
NTL.phys.data.TuesdayLake <- 
  filter(NTL.phys.data, lakename %in% ("Tuesday Lake"))
NTL.phys.data.Tuesday1900 <- filter(NTL.phys.data.TuesdayLake, year4 %in% c(1990:1999))
...
NTL.phys.data.TuesdayPipe <- 
  NTL.phys.data %>% 
  filter(lakename == "Tuesday") %>%
  filter(month(sampledate)=7) %>% 
  NTL.phys.data.TuesdayJuly <- filter(NTL.phys.data.TuesdayLake, year4 %in% c(1990:1999)) 
