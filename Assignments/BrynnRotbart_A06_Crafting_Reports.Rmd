...
kable(summary_PM25,
          label="Mean Particulates (2.5mm)",
          col.names= c("County",expression("$\\mu g/m^3$")),
          digits=2)
kable(summary_Ozone,
         label="Mean Ozone",
          col.names= c("County",expression("$\\mu g/m^3$")),
...
```{r echo=FALSE, fig.align="left", fig.cap="Ozone Levels in 2018", fig.height=4, fig.width=8, warning=FALSE}
summary_ozone2018<-
  epa_data %>% 
  filter(Year == "2018") 

summary_ozone2018$Month<-factor(summary_ozone2018$Month)

ggplot(summary_ozone2018)+
  geom_boxplot(aes(
               x=Month,
               y=Ozone))
```

```{r echo=FALSE, fig.align="left", fig.cap="Ozone Levels in 2019", fig.height=4, fig.width=8, warning=FALSE}
summary_ozone2019<-
  epa_data %>% 
  filter(Year == "2019") 

summary_ozone2019$Month<-factor(summary_ozone2019$Month)

ggplot(summary_ozone2019)+
  geom_boxplot(aes(
               x=Month,
               y=Ozone))
```
