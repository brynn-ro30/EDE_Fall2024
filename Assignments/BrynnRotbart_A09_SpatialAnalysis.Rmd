...
> ANSWER: 
> The EPSG Code is 4269 and it is geocentric coordinate reference system, which is a geogrpahic coordinate reference system. This coordinate system uses angular coordinate units. The CRS is associated with the North American Datum 1983 
...
> ANSWER:  
> longitude is x and latitude is y. This is referred to in the data set as "dec_lat_va" and "dec_long_va".
...
>ANSWER: 
>The latitude and longitude fields disappeared but a field called "geometry" has appeared with coordinate points 
...
```{r Plot the spatial features, fig.cap="Brynn Rotbart"}
#13. Plot the gage locations atop the county features
ggplot() + 
  geom_sf(data=Nebraska_Counties, fill="lavender") +
  geom_sf(data=Nebraska_Sf) +
...
FlowData <- read.csv("Data/Raw/NWIS_SiteFlowData_NE_RAW.csv",
                     stringsAsFactors= TRUE) 
...
mapView(Nebraska_Counties, col.regions="grey")+
...
#"plot data so each can be seen as different colors"??? What does "each" mean???
#directions are confusing because plotting based off the transformed UTM's does
#change the gage plot points 
counties_utm <-st_transform(Nebraska_Counties, crs = 32614)
gage_utm <-st_transform(flow_gage_sf_join, crs=32614)

#21 Plot the data
mapView(counties_utm) + mapView(gage_utm, zcol="gage_ht")
...
Lancaster_County <- counties_utm %>% filter(NAME == "Lancaster")

#23 Spatially select gages within the selected county
Lancaster_gages_intersect <- gage_utm %>%  
  st_filter(Lancaster_County, .predicate = st_intersects)

#24 Spatially select gages within 15 k of the selected county
lancaster_in15km <- gage_utm %>% 
  st_filter(st_buffer(Lancaster_County, 15000))

#25 Plot
mapView(Lancaster_County, col.region="purple")+
  mapview(Lancaster_gages_intersect, col.region ="lightpink") + 
  mapView(lancaster_in15km, col.region = "lightblue")
